...
function m() {
  for (const e of ["touchstart", "mousedown"])
    document.body.addEventListener(
      ...
      (t) => {
        i(t.target, "tap") &&
          s(t.target.href, { with: "fetch", ignoreSlowConnection: !0 })
      },
      { passive: !0 },
    )
}
      ...
      o.isIntersecting
        ? (a && clearTimeout(a),
          e.set(
            r,
            setTimeout(() => {
              n.unobserve(r), e.delete(r), s(r.href, { with: "link" })
            }, 300),
          ))
...
function S() {
  u(() => {
    for (const e of document.getElementsByTagName("a"))
      i(e, "load") && s(e.href, { with: "link" })
  })
}
function s(e, t) {
  const n = t?.ignoreSlowConnection ?? !1
  if (!L(e, n)) return
  if ((d.add(e), (t?.with ?? "link") === "link")) {
...
function L(e, t) {
  if (!navigator.onLine || (!t && v())) return !1
  try {
